<?php
/**
 * CSV export handler for Milner Stats.
 *
...
class WMS_Export {
	...
	private static function export_top_posts( string $period ) {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- streaming to php://output (stdout), not a filesystem file; WP_Filesystem does not support this.
	...
	private static function export_daily_totals( int $days ) {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- streaming to php://output (stdout); WP_Filesystem does not support this.
		$out = fopen( 'php://output', 'w' );
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fputs
		fputs( $out, "\xEF\xBB\xBF" );
	...
	private static function export_raw() {
		...
		do {
			...
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $table is $wpdb->prefix . constant, not user input.
