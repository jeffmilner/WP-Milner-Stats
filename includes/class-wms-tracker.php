...
class WMS_Tracker {
	...
	public static function record(
		...
		// 4b. Skip headless/scripted browsers — these pass bot string checks but
		//     have telltale UA signatures from automation frameworks.
		if ( self::is_headless_browser( $user_agent ) ) {
			return 'skipped';
		}
	...
	private static function is_bot( string $user_agent ): bool {
		...
		$patterns = apply_filters( 'wms_bot_patterns', [
			...
			'CCBot', 'Bytespider', 'PetalBot', 'ImagesiftBot',
			'Diffbot', 'Applebot', 'cohere-ai',
		] );
	...
	/**
	 * Detect headless and scripted browsers that use clean-looking user agents
	 * but expose themselves through other UA characteristics.
	 *
	 * These are NOT caught by the bot string list because they deliberately
		 * mimic real browser user agents. Common sources: scraping-as-a-service,
	 * content aggregators, AI training pipelines.
	 */
	private static function is_headless_browser( string $user_agent ): bool {
		if ( empty( $user_agent ) ) {
			return true;
		}

		$patterns = apply_filters( 'wms_headless_patterns', [
			// Playwright / Puppeteer / Selenium identifiers
			'headlesschrome', 'phantomjs', 'selenium', 'webdriver',
			'playwright', 'puppeteer',
			// Python HTTP libraries used for scraping
			'python-requests', 'python-urllib', 'python-httpx',
			'scrapy', 'httpie', 'aiohttp',
			// Other common scraper runtimes
			'go-http-client', 'java/', 'okhttp', 'axios/',
			'node-fetch', 'got/', 'superagent',
			// Hosting/proxy scanners
