<?php
/**
 * Admin dashboard controller for Milner Stats.
 *
...
class WMS_Admin {
	...
	public function enqueue_assets( string $hook ) {
		...
		// Chart.js — bundled locally (wp.org policy: no external script offloading).
		// File: admin/js/vendor/chart.umd.min.js  (Chart.js 4.4.3, UMD build)
		// Download: https://cdn.jsdelivr.net/npm/chart.js@4.4.3/dist/chart.umd.min.js
		wp_enqueue_script(
			'chartjs',
			WMS_PLUGIN_URL . 'admin/js/vendor/chart.umd.min.js',
			[],
			'4.4.3',
			true
		);
	...
	public function render_settings_page() {
		...
		if ( isset( $_GET['wms_test_referrer'] ) && check_admin_referer( 'wms_test_referrer' ) ) {
			...
			if ( $first_post ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- one-off admin/diagnostic query; caching not appropriate.
		...
		// One-time historical visitor data migration.
		// Recomputes is_new_visitor for all existing rows: marks the first view per
		// (ip_hash, calendar-day) as is_new_visitor = 1, all subsequent views that
		// day as 0. This corrects the inflated visitor counts recorded before v1.1.0
		// where is_new_visitor was scoped per-post instead of per-site.
		if ( isset( $_GET['wms_fix_visitors'] ) && check_admin_referer( 'wms_fix_visitors' ) ) {
			$result = self::run_visitor_migration();
			if ( is_wp_error( $result ) ) {
				echo '<div class="notice notice-error is-dismissible"><p>'
					. esc_html__( 'Migration failed: ', 'wp-milner-stats' )
				...
				echo '<div class="notice notice-success is-dismissible"><p>'
					. sprintf(
						...
						esc_html__( 'Visitor migration complete. %d rows updated.', 'wp-milner-stats' ),
						(int) $result
					)
					. '</p></div>';
			}
		...
		<div class="wrap wms-wrap">
			...
			<?php
			// Show recent user agents — helps identify scrapers that passed bot detection
			$views_table = $wpdb->prefix . WMS_TABLE_NAME;
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$recent_uas = $wpdb->get_results(
			...
			if ( $recent_uas ) :
			?>
			<h3 style="font-size:14px;margin:16px 0 6px"><?php esc_html_e( 'User Agents (Last 24h, recorded views only)', 'wp-milner-stats' ); ?></h3>
			<table class="widefat striped" style="max-width:750px;margin-bottom:16px">
				<thead><tr>
					<th><?php esc_html_e( 'User Agent', 'wp-milner-stats' ); ?></th>
					<th style="width:60px;text-align:right"><?php esc_html_e( 'Views', 'wp-milner-stats' ); ?></th>
				</tr></thead>
				<tbody>
				<?php foreach ( $recent_uas as $ua_row ) : ?>
					<tr>
						<td style="font-size:11px;word-break:break-all"><?php echo esc_html( $ua_row->user_agent ); ?></td>
						<td style="text-align:right;font-weight:600"><?php echo esc_html( number_format_i18n( $ua_row->cnt ) ); ?></td>
					</tr>
				<?php endforeach; ?>
				</tbody>
			</table>
	...
	/**
	 * Recompute is_new_visitor for all historical rows.
	 *
	 * The pre-v1.1.0 bug scoped "new visitor" per post instead of per site,
	 * so almost every row had is_new_visitor = 1. This migration corrects that
	 * by marking only the earliest view per (ip_hash, local calendar-day) as 1.
	 *
	 * Strategy (two-pass, no temp table needed):
	 *   1. Reset every row to is_new_visitor = 0.
	 *   2. Set is_new_visitor = 1 on the MIN(id) per (ip_hash, day).
	 *
	 * Uses the site timezone for day boundaries so "day" matches the dashboard.
	 *
	 * @return int|WP_Error  Number of rows updated (step 2), or WP_Error on failure.
	 */
	public static function run_visitor_migration() {
		global $wpdb;
		$table  = $wpdb->prefix . WMS_TABLE_NAME;

		// Derive site UTC offset for CONVERT_TZ (same logic as WMS_Insights)
		$tz             = wp_timezone();
		$now_local      = new \DateTimeImmutable( 'now', $tz );
		$offset_seconds = $now_local->getOffset();
		$sign           = $offset_seconds >= 0 ? '+' : '-';
		$abs            = abs( $offset_seconds );
		$tz_off         = sprintf( '%s%02d:%02d', $sign, (int) floor( $abs / 3600 ), (int) ( ( $abs % 3600 ) / 60 ) );

		// Step 1: reset all rows to 0
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$reset = $wpdb->query( "UPDATE {$table} SET is_new_visitor = 0" );
		if ( false === $reset ) {
			return new \WP_Error( 'migration_failed', $wpdb->last_error );
		}

		// Step 2: mark the first view of each ip_hash per local calendar-day as 1.
		// We find the MIN(id) per (ip_hash, local-day) via a subquery, then UPDATE.
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$updated = $wpdb->query(
			$wpdb->prepare(
				...
				"UPDATE {$table} v
				 INNER JOIN (
				     ...
				     FROM   {$table}
				     GROUP  BY ip_hash, DATE( CONVERT_TZ(viewed_at, '+00:00', %s) )
				 ) AS first_views ON v.id = first_views.first_id
				 SET v.is_new_visitor = 1",
				$tz_off
			)
		);
		...
		// Flush stats cache so the dashboard reflects the corrected numbers immediately
